    ...
    () =>
      engine.onTuner((t) => {
        ...
        if (t.freq > 0 && t.cents !== undefined) {
          ...
          needle.className = "absolute top-0 h-full w-0.5 -translate-x-1/2 bg-muted-foreground/40"
          cents.textContent = ""
        }
      }),
    [engine],
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <div className="relative h-6 w-full max-w-md rounded bg-muted/40">
        ...
        <div className="absolute left-1/2 top-0 h-full w-px bg-muted-foreground/60" />
        <div
          ref={needleRef}
      ...
      <div ref={centsRef} className="h-5 text-sm tabular-nums text-muted-foreground" />
    </div>
  )
}
